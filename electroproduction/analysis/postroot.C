...
void postroot() {
    ...
    TFile *f = TFile::Open("/Users/mr282803/Documents/rgA_outbending.root");
    // recuperer t tree
    TTree *tree = (TTree*)f->Get("tree;1");

    // Variables pour lire les branches
    double W, Q2, t;
    double MMD, Minv_pipD_pimD, Minv_Ks_KlD;
    double status_pim, status_pip, status_pr;
    ...
    TLorentzVector *pipD = nullptr;
    TLorentzVector *pimD = nullptr;
    ...
    tree->SetBranchAddress("PiPlusD", &pipD);
    tree->SetBranchAddress("PiMinusD", &pimD);

    tree->SetBranchAddress("MinvKsKlD", &Minv_Ks_KlD);
    tree->SetBranchAddress("MissingMassD", &MMD);
    tree->SetBranchAddress("MinvPiPlusMoinsD", &Minv_pipD_pimD);
    ...
    TH1F *MinvPiPlusMoins_hist = new TH1F("MinvPiPlusMoins_hist", "Invariant Mass #pi^{-} #pi^{+}", bin4, 0, 1.5);
    TH1F *MissingMass_hist = new TH1F("MissingMass_hist", "Missing Mass", bin4, 0, 1.5);
    TH1F *MinvKsKl_hist = new TH1F("MinvKsKl_hist", "Invariant Mass K_{s} K_{L}", bin4, 0.7, 1.5);

    TH2F *MinvKsKl_W = new TH2F("MinvKsKl_W", "Invariant Mass Ks Kl vs W", bin3, 1, 5, bin3, 0, 3.5); 
    TH2F *MinvKsKl_Q2 = new TH2F("MinvKsKl_Q2", "Invariant Mass Ks Kl vs Q^{2}", bin3, 0, 10, bin3, 0, 3.5); 
    TH2F *MinvKsKl_Thetae = new TH2F("MinvKsKl_Thetae", "Invariant Mass Ks Kl vs Theta electron", bin3, 0, 50, bin3, 0, 3.5); 

    TH1F *MinvPiPlusMoins_hist2 = new TH1F("MinvPiPlusMoins_hist2", "Invariant Mass #pi^{-} #pi^{+} with cut on MM", bin4, 0, 1.5);
    TH1F *MissingMass_hist2 = new TH1F("MissingMass_hist2", "Missing Mass with cut on M_{inv} of #pi^{+}#pi^{-}", bin4, 0, 1.5);

    TH2F *theta_p_el = new TH2F("theta_p_el", "Theta vs p for electron", bin5, 0, 45, bin5, 0, 10);
    ...
    TH2F *theta_p_el2 = new TH2F("theta_p_el2", "Theta vs p for electron with all cuts", bin5, 0, 45, bin5, 0, 10);
    ...
    for (Long64_t i = 0; i < nEntries; i++) {
        ...
        MinvKsKl_W->Fill(W, Minv_Ks_KlD);
        MinvKsKl_Q2->Fill(Q2, Minv_Ks_KlD);
        MinvKsKl_Thetae->Fill(el->Theta()*(180/3.14), Minv_Ks_KlD);

        theta_p_el->Fill(el->Theta()*(180/3.14), el->P());
        theta_p_pr->Fill(pr->Theta()*(180/3.14), pr->P());
        theta_p_pim->Fill(pimD->Theta()*(180/3.14), pimD->P());
        theta_p_pip->Fill(pipD->Theta()*(180/3.14), pipD->P());
        ...
        MinvPiPlusMoins_hist->Fill(Minv_pipD_pimD);
        MissingMass_hist->Fill(MMD);


        if(MMD < 0.6 && MMD > 0.4 && R_pip_pim < 2.0 && R_Ks_e < 5.0 && R_Ks_e > 1.5 && status_pim >= 2000 && status_pim <= 2999 && status_pip >= 2000 && status_pip <= 2999){ // cut FD + cut missing mass

                    MinvPiPlusMoins_hist2->Fill(Minv_pipD_pimD);
                    MKs_R_Ks_e2->Fill(R_Ks_e, Minv_pipD_pimD);
                    MKs_R_pip_pim2->Fill(R_pip_pim, Minv_pipD_pimD);

        }

        if(Minv_pipD_pimD < 0.6 && Minv_pipD_pimD > 0.4 && R_pip_pim < 2.0 && R_Ks_e < 5.0 && R_Ks_e > 1.5 && status_pim >= 2000 && status_pim <= 2999 && status_pip >= 2000 && status_pip <= 2999){ // cut FD + cut masse invariante pi+ pi-

                    MissingMass_hist2->Fill(MMD);
        ...
        if(MMD < 0.6 && MMD > 0.4 && Minv_pipD_pimD > 0.4 && Minv_pipD_pimD < 0.6 && R_pip_pim < 2.0 && R_Ks_e < 5.0 && R_Ks_e > 1.5 && status_pim >= 2000 && status_pim <= 2999 &&status_pip >= 2000 && status_pip <= 2999){ // cut FD + cut masse invariante pi+ pi- + cut missing mass

                    MinvKsKl_hist->Fill(Minv_Ks_KlD);
                    MPhi_R_Ks_e2->Fill(R_Ks_e, Minv_Ks_KlD);
                    MPhi_R_pip_pim2->Fill(R_pip_pim, Minv_Ks_KlD);

        }

        if(Minv_Ks_KlD < 1.05 && Minv_Ks_KlD > 0.95 && MMD < 0.6 && MMD > 0.4 && Minv_pipD_pimD > 0.4 && Minv_pipD_pimD < 0.6 && R_pip_pim < 2.0 && R_Ks_e < 5.0 && R_Ks_e > 1.5 && status_pim >= 2000 && status_pim <= 2999 &&status_pip >= 2000 && status_pip <= 2999){
                        
                    theta_p_el2->Fill(el->Theta()*(180/3.14), el->P());
                    theta_p_pr2->Fill(pr->Theta()*(180/3.14), pr->P());
                    theta_p_pim2->Fill(pimD->Theta()*(180/3.14), pimD->P());
                    theta_p_pip2->Fill(pipD->Theta()*(180/3.14), pipD->P());
        }
    ...
    TString pdfFile = "analysisRgA_outbending.pdf";
    ...
    latex.DrawLatexNDC(0.1, 0.9, "File : RG-A outbending with correction (veronique code on pi+ pi-)");
    latex.DrawLatexNDC(0.1, 0.8, "Number of hipo file : 50");

    latex.DrawLatexNDC(0.1, 0.6, "Summary of cuts for the next plots:");
    latex.DrawLatexNDC(0.1, 0.5, "- Cut already present in nSidis files (like p_e^{-} > 2 GeV, Q^{2} > 1 GeV)");
    latex.DrawLatexNDC(0.1, 0.4, "- Only 1 proton, #pi^{+} >= 1, #pi^{-} >= 1, e^{-} >= 1");
    ...
    t_hist->Draw(); c->Print(pdfFile);
    W_hist->Draw(); c->Print(pdfFile);
    ...
    theta_p_el->Draw("COLZ"); c->Print(pdfFile);
    ...
    MinvKsKl_W->Draw("COLZ"); c->Print(pdfFile);
    MinvKsKl_Q2->Draw("COLZ"); c->Print(pdfFile);
    ...
    latex.DrawLatexNDC(0.1, 0.8, "- Add a cut on missing mass : 0.4 < MM < 0.6 GeV");
    ...
    latex.DrawLatexNDC(0.1, 0.7, "  0.4 < M_{inv} < 0.6 GeV");
    

    c->Print(pdfFile);


    MissingMass_hist2->Draw();
    ...
    latex.DrawLatexNDC(0.1, 0.9, "Test on p vs #theta for all particle to see if the absence of signal");
    latex.DrawLatexNDC(0.1, 0.8, "comes from acceptance : ");
    latex.DrawLatexNDC(0.1, 0.7, "- All cuts + 0.8 < M_{#phi} < 1.2 GeV");
    

    c->Print(pdfFile);

    theta_p_el2->Draw("COLZ"); c->Print(pdfFile);
    ...
    cout << "PDF rg-A correction ecris" <<endl;
