...
void postrootMC() {
    ...
    TFile *f = TFile::Open("/Users/mr282803/Documents/generateur/inbendingMC.root");
    // recuperer t tree
   ...
   //setTitles(Dist2, "#sqrt{(v_{#pi^{+}_{x}} - v_{#pi^{-}_{x}})^{2} + (v_{#pi^{+}_{y}} - v_{#pi^{-}_{y}})^{2} + (v_{#pi^{+}_{z}} - v_{#pi^{-}_{z}})^{2}} [cm]", "");
    ...
    TString pdfFile = "analysisMC_inbending.pdf";
    ...
    latex.DrawLatexNDC(0.1, 0.9, "File : MC inbending without vertex correction");
